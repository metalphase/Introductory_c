...
{   
    ...
    {
        if (strlen(current_word) >= min_identical)
        {
            ...
            {
                printf("append: %s \n", current_word);
                append(&head_ref, current_word);
                printList(head_ref);
                printf("\n");
            }
            else if (strcmp(head_ref->word, current_word) > 0)
            {
                printf("push: %s ", current_word);
                push(&head_ref, current_word);
                printList(head_ref);
                printf("\n");
            }
            else
            {
                ...
                while(strcmp(current_node->word, current_word) < 0 && current_node->next != NULL)
                {
                    previous_node = current_node;
                    current_node = current_node->next;
                }
                
                printf("current_node: %p ", current_node);
                printf("previous_node: %p \n", previous_node);
                
                if(current_node->next == NULL)
                    append(&head_ref, current_word);
                else
                    insertAfter(previous_node, current_word);
...
void push(struct Node **head_ref, char *new_word)
{
    ...
    new_node->word = (char *)malloc(word_len * sizeof(char *));
